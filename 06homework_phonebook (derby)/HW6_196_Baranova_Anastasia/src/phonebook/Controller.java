...
public class Controller implements Initializable {
    ...
    /**
     * Phonebook database to store user's contacts.
     */
    private final PhonebookDatabase database = new PhonebookDatabase();


    /**
     * Looks for conflicts in the existing list of contacts with passed contact.
    ...
    private Contact solveConflict(Contact contact, Contact except) {
        var conflict = database.findConflict(contact);
        if (conflict != null && !conflict.equals(except)) {
            ...
            database.deleteContact(conflict);
    ...
    private void solveConflictsAndAdd(Object[] newContacts) {
        for (var c : newContacts) {
            var toAdd = solveConflict((Contact) c, null);
            if (toAdd != null) {
                database.addContact(toAdd);
            }
        }
        refreshTable();
    }
    ...
    private void editContact() {
        ...
        database.editContact(old, edited);
        refreshTable();
    ...
    public void addContact() {
        ...
        var result = (Contact) newContact;
        database.addContact(result);
        refreshTable();
    }
    ...
    private void exportContacts() {
        UIUtils.openWindowForResult(getClass().getResource("exportimport/exportForm.fxml"),
                true, database.importContacts().toArray());
    }
    ...
    private void filter(String key) {
        var filtered = FXCollections.observableArrayList(database.filter(key));
    ...
    private void refreshTable() {
        if (search.getText() != null && !search.getText().isBlank()) {
            ...
            table.setItems(FXCollections.observableArrayList(database.importContacts()));
        }
        table.refresh();
    }
